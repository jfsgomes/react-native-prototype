...
export default class BitcoinScreen extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      bitstampPrice: 0,
  ...
  updateAllPrices() {
    this.getBTCPrice("Bitstamp");
    this.getBTCPrice("Kraken");
    this.getBTCPrice("GDAX");
  }
  ...
  getBTCPrice(exchange, key) {
    switch (exchange) {
      case "Bitstamp":
        return this.fetchPrice("https://www.bitstamp.net/api/v2/ticker/btceur/").then(
          (responseJson) => this.setState({ bitstampPrice: this.numberWithTwoDecimals(responseJson.last) })
        );
        break;
      case "Kraken":
        return this.fetchPrice("https://api.kraken.com/0/public/Ticker?pair=XXBTZEUR").then(
          (responseJson) => this.setState({ krakenPrice: this.numberWithTwoDecimals(responseJson.result.XXBTZEUR.c[0]) })
        );
        break;
      case "GDAX":
        return this.fetchPrice("https://api.pro.coinbase.com/products/BTC-EUR/ticker").then(
